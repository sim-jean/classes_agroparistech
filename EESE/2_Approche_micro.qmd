...
### Somme horizontale: raisonnement graphique
...
data %>%
  ggplot(aes(y = Prix, x = Quantité, color = Demandes))+
  geom_line()+
  xlim(0.1,110)+
  custom_theme()
...
### 2. Offre individuelle, offre collective

Pour un prix donné, on somme les quantités offertes : 
...
D <- 1   # Offre privée (PMC) : p = C + D q

tau <- 8            # Dommage marginal (SMC = PMC + tau)

# ---- Fonctions ----
MB  <- function(q) A - B*q
PMC <- function(q) C + 6/10*q
...
q1_inter = function(p) 4/6*p
q2_inter = function(p) p
...
ggplot() +
  ...
  geom_segment(aes(x = q1_inter(10), xend = q1_inter(10), y=0, yend =10), linetype = "dotted")+
  #annotate("label", x = q1_inter(10), y = -Inf, label = "N", vjust = -0.5)+
  geom_segment(aes(x = q2_inter(10), xend = q2_inter(10), y=0, yend =10), linetype = "dotted")+
  #annotate("label", x = q2_inter(10), y = Inf, label = "N", vjust = 1.1)+
  geom_segment(aes(x = q2_inter(10)+q1_inter(10), xend=q2_inter(10)+q1_inter(10), y=0, yend=10), linetype="dotted")+
...
ggplot() +
  geom_line(data = demand, aes(x = Quantite, y = Prix, color = Courbe), size=1) +
  geom_line(data = supply, aes(x = Quantite, y = Prix, color = Courbe), size=1) +
  geom_polygon(data = cs_poly, aes(x = Quantite, y = Prix), fill="orange", alpha=0.4) +
  geom_polygon(data = ps_poly, aes(x = Quantite, y = Prix), fill="lightblue", alpha=0.4) +
  geom_point(aes(x = q_star, y = p_star), size=1, color="black") +
  geom_vline(xintercept = q_star, linetype="dashed", linewidth = .4) +
  geom_hline(yintercept = p_star, linetype="dashed", linewidth = .4) +
  labs(
...
-   Le coût (ou bénéfice) **social** des actions diffère du coût (ou bénéfice) **privé**
    -   Il faut prendre ces fonctions dans l'analyse!
...
Dans l'analyse, on intègre donc : 

::: incremental 

- Le bénéfice marginal social lié à une action i.e. qui échoit à d'autres que ses producteurs : $MB(Q)$
- Le dommage marginal social lié à une action : $MD(Q)$

:::

$$
S_T = \int_0^Q DMP(q) dq - \int_0^Q C'(q)dq + \int_0^Q MB(q)dq - \int_0^Q MD(q)dq 
$$
...
Si le coût marginal de production est de $Cm(Q) = Q$, l'optimum est :
